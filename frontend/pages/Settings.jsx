...
import { Save, User, Mail, Lock, Trash2, AlertTriangle, Key, Eye, EyeOff, X } from 'lucide-react';
...
function Settings() {
  ...
  // Validate: If new password is provided, current password is required
  const isValid = hasChanges && (
  ...
  const handleUpdate = (e) => {
    ...
    try {
      // Find current user in storage and update
      ...
      // Filter out old user record and add new one
      const otherUsers = allUsers.filter(u => u.id !== user.id);
      localStorage.setItem('codeguard_users', JSON.stringify([...otherUsers, updatedUser]));
  ...
  const confirmDeleteAccount = () => {
    if (deleteConfirmText === 'DELETE') {
      const allUsers = storageService.users.getAll().filter(u => u.id !== user?.id);
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in">
      <div>
          ...
          <p className="text-slate-500 mt-1 text-sm md:text-base">Manage your personal information.</p>
      </div>
      
      {message && (
          <div className={`p-4 rounded-lg text-sm border ${message.type === 'success' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
              {message.text}
          </div>
      )}
        ...
        <div className="fixed inset-0 md:left-72 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-surface rounded-2xl border border-red-500/30 w-full max-w-md p-6 shadow-2xl relative z-[80]">
            ...
            <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 text-red-500 rounded-lg text-xs flex items-start gap-2">
              ...
              <p>
                This action <span className="font-bold">cannot be undone</span>. All your projects, files, and scan history will be permanently deleted.
              </p>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); confirmDeleteAccount(); }} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">
                  Type <span className="text-white bg-slate-800 px-1.5 py-0.5 rounded font-mono normal-case">DELETE</span> to confirm
                </label>
                <input
                  type="text"
                  required
